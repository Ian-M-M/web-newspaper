...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $conn = new mysqli("localhost", "root", "", "newspapaer");
    ...
    // Reemplazar la fecha en la cabecera
    $html = preg_replace('/Edición del.*?<\/div>/', "Edición del $fecha</div>", $html);
    ...
    foreach ($noticias as $i => $noticia) {
        ...
        if ($noticia["isPrimaria"]) {
            ...
            $html = preg_replace('/(<img[^>]+alt="Noticia principal"[^>]+src=")[^"]+/', "$1$imagen", $html);
            $html = preg_replace('/<a[^>]*class="text-2xl[^>]*>.*?<\/a>/', "<a href=\"#\" class=\"text-2xl font-bold block mb-2 hover:underline\">$titulo</a>", $html);
            $html = preg_replace('/<p>\s*Resumen breve de la noticia principal.*?<\/p>/', "<p>$descripcion</p>", $html);
        } else {
            // Noticias secundarias: usa comentarios para ubicar
            $index = ++$secundarias;
            $html = preg_replace_callback('/<!-- Noticia secundaria ' . $index . ' -->(.*?)<\/div>\s*<\/div>/s', function ($matches) use ($titulo, $descripcion, $imagen) {
                $seccion = $matches[1];
                $seccion = preg_replace('/<img[^>]+src="[^"]+/', "<img src=\"$imagen\"", $seccion);
                $seccion = preg_replace('/<a[^>]*class="text-xl[^>]*>.*?<\/a>/', "<a href=\"#\" class=\"text-xl font-semibold block mb-1 hover:underline\">$titulo</a>", $seccion);
